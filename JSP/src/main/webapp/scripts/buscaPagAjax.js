    ...
    var nomeBusca = document.getElementById('nomeBusca').value;
	 ...
	 $.ajax({	     
		 ...
		 $("#ulPaginacaoUserAjax > li").remove();
		  ...
		  for(var p = 0; p < json.length; p++){
		      $('#tabelaresultados > tbody').append('<tr><td style="height: 20px;">' + 
						json[p].cep + '</td><td> ' + json[p].id + '</td><td> ' + json[p].nome + 
						'</td><td><button id="editar" onclick="editar('+json[p].id+')" type="button" style="margin: -15px;" class="btn btn-info">Ver</button></td></tr>');
		  }
		    ...
		    var totalPagina = xhr.getResponseHeader("totalPagina");
			  ...
			  for (var p = 0; p < totalPagina; p++){
			      ...
			      $("#ulPaginacaoUserAjax").append('<li class="page-item"><a class="page-link" href="#" onclick="buscaPagAjax(\''+url+'\')">'+ (p + 1) +'</a></li>'); 
			  }
	     }
	 }).fail(function(xhr, status, errorThrown){
